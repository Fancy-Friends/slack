...
npm run provider -- slack            # regenerate this repo
npm run provider -- slack --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/slack-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/slack-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/slack-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-slack` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/slack/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/slack` |
| npm | `@particle-academy/slack-ui`, `@particle-academy/slack-js` |
| Packagist | `particle-academy/slack-php` |
| PyPI | `fancy-slack` |
...
**Default to plain HTTP.** A Slack SDK is third-party code and is subject to
